...
function getActiveUserValues(users: User[]): number[] {
  return users.filter((user) => user.active).map((user) => user.value)
}
```
...
const isEligibleForAccess =
  user.age >= 18 && user.status === "verified" && !user.banned
...
describe("UserService", () => {
  describe("createUser", () => {
    ...
    it("should throw error for invalid email format", async () => {
      await expect(service.createUser("invalid-email")).rejects.toThrow(
        ValidationError,
      )
    })
